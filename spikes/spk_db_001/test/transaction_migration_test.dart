...
void main() {
  ...
  test('Foreign key constraints are enforced', () async {
    ...
    expect(
      () async => await dbService.db.into(dbService.db.commercialProfiles).insert(
  ...
  test('Transactions can be rolled back on error', () async {
    ...
    expect(contacts, isEmpty, reason: 'Contact should not have been inserted because transaction rolled back');
  });
  ...
  test('Migration from schema v1 to v2', () async {
    ...
    rawDb.execute('CREATE TABLE contacts (id TEXT NOT NULL, organization_id TEXT NOT NULL, phone_normalized TEXT NOT NULL, display_name TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, record_version INTEGER NOT NULL DEFAULT 1, PRIMARY KEY (id));');
    ...
    await appDb.into(appDb.contacts).insert(ContactsCompanion.insert(id: 'c1', organizationId: 'org1', phoneNormalized: '123', createdAt: 1, updatedAt: 1));
    await appDb.into(appDb.commercialProfiles).insert(CommercialProfilesCompanion.insert(id: 'p1', organizationId: 'org1', contactId: 'c1', prospectStatus: 'new', firstContactAt: 1, isClient: const drift.Value(true), createdAt: 1, updatedAt: 1));
